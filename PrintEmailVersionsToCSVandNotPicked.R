...
#COMBINE BOTH LISTS AND SPIT OUT LOSERS
#######################################
win_and_wait<-bind_rows(private_winners, private_waiters)
win_and_wait<-subset(win_and_wait, select = -c(Waitlisted_Name,Num,fullname) )
losers<-anti_join(df, win_and_wait)
write.csv(losers,".\\HL2024NotPicked.csv")
